...
it('🟢 Case 1: Add Pattern with Private Image and Verify Detail Page', async () => {
    ...
    try {
        ...
        if (!await addToStash.isDisplayed()) throw new Error("Add to Stash NOT visible");

        // 2. Search & Add Pattern
        ...
        if (!await searchPattern.isDisplayed()) throw new Error("Search screen NOT visible");
        ...
        // 5. Private Image > Photo Library
        console.log('Step 5: Selecting Private Image');
        await $('android=new UiSelector().description("Upload").instance(1)').click();
        ...
        // 7. Content tab
        console.log('Step 7: Clicking Content tab');
        await $('android=new UiSelector().text("Content")').click();
        await driver.pause(1500);

        // 8. Upload text button
        console.log('Step 8: Clicking Upload text button');
        await $('android=new UiSelector().text("Upload")').click();
        await driver.pause(1500);

        // 9. Pointer Upload Button
        console.log('Step 9: Pointer Upload Button Tap');
        await uploadButtonClick();
        await driver.pause(4000);

        // 10. Verify privacy warning
        console.log('Step 10: Verify Privacy Warning');
        const privacy = await $('android=new UiSelector().text("Remember: Keep instructions\nand copyrighted details Private")');
        if (!await privacy.isDisplayed()) throw new Error("Privacy Warning NOT visible");

        // 11. GOT IT (your logic)
        console.log('Step 11: Handling Got it');
        try {
        ...
        // 12. Save button pointer
        console.log('Step 12: Clicking Save (Pointer Logic)');
        await saveButtonClick();
        await driver.pause(5000);

        console.log('Step 13: Waiting...');
        ...
        // 15. Lock icon & private message
        console.log('Step 15: Verifying Private Lock Message');
       // await $('android=new UiSelector().className("android.widget.ImageView").instance(1)').click();

        const privateMsg = await $('android=new UiSelector().text("Only you can see this pattern.")');
        if (!await privateMsg.isDisplayed()) throw new Error("Private message NOT visible");
        ...
        console.log('❌ Case 1 Failed:', err.message);
        throw err;
    }
